const Card = ({ children, className = "", hover = true, glow = false }) => (
  <div className={`relative ${glow ? 'group' : ''}`}>
    ...
    <div className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-stone-200 dark:border-gray-700 relative ${hover ? 'hover:shadow-xl transition-all duration-300' : ''} ${className}`}>
      {children}
    </div>
  </div>
);
