    ...
    <div className="relative inline-block w-full max-w-[12rem]" ref={dropdownRef}>
      ...
      >
        <span className="flex items-center space-x-2">
          ...
          <span className="truncate text-gray-700 dark:text-stone-200 text-sm sm:text-base">
            {selected.name}
          </span>
        </span>
        <ChevronDown
      ...
      {isOpen && (
        <ul
          role="listbox"
          className="absolute z-50 mt-2 max-h-60 w-full overflow-auto rounded-lg border border-stone-200 dark:border-dark-700 bg-white dark:bg-dark-800 shadow-lg ring-1 ring-black/5 focus:outline-none text-sm sm:text-base"
        >
          {languages.map((lang) => (
            <li
              key={lang.code}
              ...
              className={`cursor-pointer select-none py-2 px-3 flex items-center space-x-2 ${
                selected.code === lang.code
                  ...
                  : "text-gray-700 dark:text-stone-200 hover:bg-stone-100 dark:hover:bg-dark-700"
              }`}
            >
